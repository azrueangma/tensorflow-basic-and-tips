...
with tf.Session() as sess:
    ...
    ckpt = tf.train.get_checkpoint_state('./model6')
    ...
    te_a, te_p = sess.run([accuracy, predict], feed_dict={X: x_test, Y: y_test})

    print("Test Accuracy : {:.2%}".format(te_a))

    fig = plt.figure()
    check_step = 0
    #실제 값을 비교하기 위하여 flat하게 만들어준다.
    te_r = np.reshape(y_test, -1)

    #잘못 예측된 10개만 비교해본다.
    moving_step=0
    while(check_step<10):
        if not te_p[moving_step] == te_r[moving_step]:
            subplot = fig.add_subplot(2, 5, check_step+1)
            check_step+=1
            #눈금을 지운다.
            subplot.set_xticks([])
            subplot.set_yticks([])
            subplot.set_title('%d' % te_p[moving_step])
            subplot.imshow(x_test[moving_step].reshape((28, 28)), cmap=plt.cm.gray_r)
        moving_step+=1
    plt.show()