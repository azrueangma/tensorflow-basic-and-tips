...
x_train, x_validation, x_test, y_train, y_validation, y_test=load_data.load_mnist('./data/mnist/', seed=0, as_image=True, scaling=True)
...
flat_size = tf.reduce_prod(tf.stack([p3.get_shape()[i] for i in range(1, 4)]))
conv_flat = tf.reshape(p3, [-1,4*4*128])
f1 =  relu_layer(conv_flat, 1024, name='FC_Relu')
